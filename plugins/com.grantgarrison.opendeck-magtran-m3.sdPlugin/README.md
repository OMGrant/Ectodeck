...
- `iLook`: the preset chosen with the **Background preset** action, in steps from the start (negative after stepping back). The deck's dials never reach a background by themselves; people put the Background preset action on a dial or key when they want that.
...
- `ectodeck:look` with `{ steps }`, from the Background preset action: move that many looks forward (or back, if negative).
